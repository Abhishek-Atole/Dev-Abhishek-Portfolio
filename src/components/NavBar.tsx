...
const NavBar = () => {
	...
	const navItemVariants = {
		hover: {
			scale: 1.05,
			backgroundColor: "rgba(59, 130, 246, 0.1)", // equivalent to primary/10
			transition: { duration: 0.2 },
		},
	};
	...
	const handleNavLinkClick = (e: React.MouseEvent, targetId: string) => {
		...
		if (element) {
			const navbarHeight = 96; // 6rem = 96px
		...
		// else if (targetId === "overview") setActiveLanguage("");
	};
	...
	return (
		<>
			<motion.header
				...
				className={cn(
					"fixed top-0 w-full transition-all duration-300 py-3 sm:py-4 navbar-fixed", // Add navbar-fixed class
